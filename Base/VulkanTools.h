...
namespace vks
{
	namespace tools
	{
		...
		template<typename T>
		inline void hashCombine(std::size_t& seed, T value)
		{
			seed ^= std::hash<T>{}(value) + 0x9e3779b97f4a7c15ULL + (seed << 6) + (seed >> 2);
		}

		/** @brief Returns true if the format has a depth component */
		...
		/** @brief Returns true if the format has a depth and/or stencil component */
		inline bool isDepthStencil(VkFormat format)
		{
			return(hasDepth(format) || hasStencil(format));
		}

		inline VkImageAspectFlags getAspectFromFormat(VkFormat format)
		{
			VkImageAspectFlags aspectMask = VK_FLAGS_NONE;

			if(tools::hasDepth(format))
			{
				aspectMask |= VK_IMAGE_ASPECT_DEPTH_BIT;
			}

			if(tools::hasStencil(format))
			{
